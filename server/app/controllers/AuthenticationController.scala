...
//       invalid => Future.successful(BadRequest(Json.obj("status" -> "error", "message" -> "Invalid JSON format"))),
//       loginData => userModel.validateUser(loginData.email, loginData.password).map {
//         case Some(userId) => Ok(Json.obj("status" -> "success", "message" -> "Login successful")).withSession("email" -> loginData.email)
//         case None => BadRequest(Json.obj("status" -> "error", "message" -> "Invalid credentials"))
//       }
//     )
//   }
...
//       errors => Future.successful(BadRequest(Json.obj("status" -> "error", "message" -> "Invalid JSON format"))),
//       registrationData => userModel.createUser(registrationData.name, registrationData.email, registrationData.password, Some(registrationData.role)).map {
//         case Some(userId) => Ok(Json.obj("status" -> "success", "message" -> "User registered successfully", "userId" -> userId))
//         case None => BadRequest(Json.obj("status" -> "error", "message" -> "User already exists"))
//       }
//     )
//   }

//   def logout = Action { implicit request =>
//     Redirect(routes.AuthenticationController.loginPage).withNewSession.flashing("success" -> "You've been logged out")
//   }
// }
